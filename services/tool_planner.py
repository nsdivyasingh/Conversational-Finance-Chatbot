...
def validate_llm_plan(plan, employee_id):
    ...
    # Allow null month/year so the global default DB lookup can fill it in
...
def plan_tool(parsed_query: dict, employee_id: int, user_query: str = "") -> dict:
    ...
    # -----------------------------
    # FIELD REQUEST TAKES TOP PRIORITY
    # (e.g. "paid days", "basic", "hra", "net pay", etc.)
    # -----------------------------
    if field_request:
    ...
    # -----------------------------
    if "compare" in q or intent == "comparison" or parsed_query.get("query_type") == "comparison":
        return {
            "tool": "analyze_salary",
            "params": {
                "employee_id": employee_id,
    ...
    # (field_request already handled at top of function)
